...
public class RequestedOrderPresenter implements RequestedOrderMvpContract {
    RequestedOrderView mView;
    ApiRequestManager apiRequestObject;

    public RequestedOrderPresenter(RequestedOrderView mView, ApiRequestManager apiRequestObject) {
        this.mView = mView;
        this.apiRequestObject = apiRequestObject;
    }

    @Override
    public void fetchRequestedOrders() {
        apiRequestObject.fetchRequestedOrders(new FetchRequestedOrdersCallback(mView), CONSTANTS.NetworkRequestTags.FETCH_REQUESTED_ORDERS);
    }

    private static class FetchRequestedOrdersCallback implements VolleySeverRequest.VolleyResponseCallback {

        private WeakReference<RequestedOrderView> mView;
        private ArrayList<Order> requestedOrderList = new ArrayList<>();

        public FetchRequestedOrdersCallback(RequestedOrderView view) {
        ...
        public void onSuccess(JSONObject response) {
            try {
                ...
                RequestedOrderView view = mView.get();
                if (view != null) {
                    view.onRequestedOrdersFetched(requestedOrderList);
                }
        ...
        private void parseJsonResponse(JSONObject response) throws JSONException {
            ...
            for (int x = 0; x < orderArray.length(); x++) {
                ...
                Order order = new Order(orderId, shopperName, productName, deliverFrom, deliverTo, deliverBefore, price, orderImage, orderDateTime, null);
                requestedOrderList.add(order);
            }
        }
    }
}
