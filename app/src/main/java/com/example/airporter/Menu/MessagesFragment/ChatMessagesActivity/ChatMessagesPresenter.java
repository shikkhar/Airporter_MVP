...
public class ChatMessagesPresenter implements ChatMessagesMvpContract {
    ...
    public ChatMessagesPresenter(ChatMessagesView mView, ApiRequestManager apiRequestObject) {
        this.mView = mView;
    ...
    @Override
    public void sendChatMessage(String fcmToken, String senderName, String messageToSend, String bidderId, String shopperId) {
        apiRequestObject.sendChatMessage(fcmToken, senderName, messageToSend, bidderId,
                shopperId, new SendChatMessageCallback(mView), SEND_CHAT_MESSAGE);
    }
    ...
    private static class FetchChatMessagesCallback implements VolleySeverRequest.VolleyResponseCallback {
        ...
        private void parseJsonResponse(JSONObject response) throws JSONException {
            ...
            for (int x = 0; x < dataArray.length(); x++) {
                ...
                ChatMessage chatMessage = new ChatMessage(message, null, null, senderId);
    ...
    private static class UploadChatMessageCallback implements VolleySeverRequest.VolleyResponseCallback {
        ...
        private void parseJsonResponse(JSONObject response) throws JSONException {
            ...
            if (success.equals(CONSTANTS.NetworRequestResponse.SUCCESS)) {
                chatMessage = new ChatMessage(messageToSend, null, null, userId);
            }
        }
    }
